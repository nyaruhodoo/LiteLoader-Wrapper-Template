    ...
    return new Proxy(instance, {
      get: (_, p: string, receiver) => {
        ...
        return (...args) => {
          ...
          args =
            (() => {
              if (!this.config.eventInterceptors?.[key]) return
              if (Array.isArray(this.config.eventInterceptors[key])) {
                return this.config.eventInterceptors[key].reduce((acc, interceptor) => {
          ...
          applyRet =
            (() => {
              if (!this.config.eventInterceptors?.[`${key}:response`]) return
              if (Array.isArray(this.config.eventInterceptors[`${key}:response`])) {
                return this.config.eventInterceptors[`${key}:response`].reduce((acc, interceptor) => {
                  return interceptor(acc)
                }, applyRet)
              }
              return this.config.eventInterceptors?.[`${key}:response`]?.(applyRet)
            })() ?? applyRet
